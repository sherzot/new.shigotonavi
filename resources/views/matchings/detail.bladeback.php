...
@section('content')
    ...
    <div class="container">
        ...
        <div class="container my-3" id="offer">
            <div class="row d-flex flex-wrap justify-content-center gap-2">
                ...
                <div class="col-12 col-md-auto">
                    <form action="{{ route('offer.regist', ['id' => $job->order_code]) }}" method="POST" onsubmit="checkResumeBeforeOffer(event)">
                        @csrf
                        @if ($isOffer)
                        ...
                        @elseif (!$isOffer && !$hasActiveOffer)
                            <button type="submit" class="btn btn-main-theme w-100 py-3 px-5 shadow" name="offer">オファー</button>
                        @else
        ...
        <script>
            function checkResumeBeforeOffer(event) {
                event.preventDefault();
                let hasResume = @json($hasResume);
        
                if (!hasResume) {
                    if (confirm("オファーを送る前に履歴書と職務経歴書も一緒に送付させていただきますので、履歴書と職務経歴書の内容をご記入ください。")) {
                        window.location.href = "{{ route('resume') }}";
                    }
                } else {
                    event.target.submit();
                }
