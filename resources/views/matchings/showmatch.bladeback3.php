                ...
                <div class="col-lg-9 col-md-12">
                    ...
                    <div class="row g-4">
                        ...
                        {{--  @if (!isset($matchingJobs) || (is_iterable($matchingJobs) && count($matchingJobs) === 0))
                            <div class="alert alert-info text-center">希望条件を変更してマッチング求人件数を増やす。</div>
                        @endif  --}}
                        @if (!isset($matchingJobs) || (is_iterable($matchingJobs) && count($matchingJobs) <= 6))
                            <h3 class="text-center mb-4 mt-5 pt-3">マッチングされた求人票一覧</h3>
                    ...
                    @endif
                    @if (!isset($matchingJobs) || (is_iterable($matchingJobs) && count($matchingJobs) >= 6))
                        <div class="alert alert-info text-center fs-f24">希望条件を変更してマッチング求人件数を絞り込む。</div>
                    @endif
                    @if (!isset($matchingJobs) || (is_iterable($matchingJobs) && count($matchingJobs) < 6))
                        <div class="alert alert-info text-center fs-f24">希望条件を変更してマッチング求人件数を増やす。</div>
                    @endif

                    <form id="filterForm" action="{{ route('matchings.matchRealTime') }}" method="GET">
                        @csrf
                        @if ($errors->any())
                        ...
                        @if (isset($matchingJobs) && $matchingJobs)
                            <div class="d-grid">
                                <button class="btn btn-main-theme btn-lg w-100" type="submit">求人票一覧を更新する</button>
                            </div>
                        @else
                            <div class="d-grid">
                                <button class="btn btn-main-theme btn-lg w-100" type="submit">検索する</button>
                            </div>
                        @endif
                    </form>
