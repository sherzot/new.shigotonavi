...
@section('content')
    <div class="container py-5">
        <h2 class="text-center mb-4">基本情報変更</h2>
        ...
        <form action="{{ route('matchings.update') }}" method="POST">
            ...
            <!-- 生年月日 -->
            <div class="mb-4">
                <label class="form-label">生年月日：<small class="text-main-theme">※西暦 数字のみ入力（例：19710401）</small></label>
                <input type="text" name="birthday" class="form-control border-dark"
                    value="{{ old('birthday', isset($person->birthday) ? \Carbon\Carbon::parse($person->birthday)->format('Ymd') : '') }}"
                    maxlength="8" pattern="\d{8}" placeholder="例: 19710401">
            </div>
            @error('birthday')
                <div class="alert　text-main-theme">{{ $message }}</div>
            @enderror
            ...
            <div class="mb-4">
                <label class="text-main-theme">登録済み資格情報</label>

                @if (!empty($savedLicenses) && collect($savedLicenses)->isNotEmpty())
                    @foreach ($savedLicenses as $license)
                        {{--  <div class="d-flex align-items-center mb-2">
                    <input type="hidden" name="saved_license_ids[]" value="{{ $license->id }}">
                    
                    <!-- ✅ Guruh nomi (group_code dan) -->
                    <input type="text" class="form-control me-2" value="{{ $license->group_code }}" readonly >

                    <!-- ✅ Kategoriya nomi (category_code dan) -->
                    <input type="text" class="form-control me-2" value="{{ $license->category_code }}" readonly >

                    <!-- ✅ Litsenziya nomi (code dan) -->
                    <input type="text" class="form-control me-2" value="{{ $license->remark }}" readonly >

                    <button type="button" class="btn btn-danger btn-sm remove-license"
                        data-id="{{ $license->id }}">削除</button>
                </div>  --}}
                    @endforeach
                @else
            ...
            <!-- 希望給与 -->
            <div class="mb-4">
                <label for="desired_salary_type" class="form-label">希望給与タイプ：</label>
                <div class="row">
                    <!-- 年収 (Yillik maosh) -->
            ...
            <div class="row g-3 mt-3">
                ...
                <div class="col-6 justfiy_">
                    <button type="submit" class="btn btn-main-theme w-100">更新する</button>
                </div>
            </div>
        </form>
    </div>
