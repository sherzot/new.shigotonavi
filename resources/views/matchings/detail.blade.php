...
<div class="container">
    ...
    // Rasm holatini aniqlash
    $offerImage = 'sn-web-button-offer-off.webp';
    $isActive = false;

    if (!$isOffer && !$hasActiveOffer && $hasResume && ($isCanceled || $isCompleted || !$isCanceled)) {
    $offerImage = 'sn-web-button-offer.webp';
    $isActive = true;
    }
    @endphp
    ...
    <div class="container my-3" id="offer">
        <div class="row d-flex flex-wrap justify-content-center gap-2">
            <div class="col-12 col-md-auto text-center">
                <form action="{{ route('offer.regist.submit', ['id' => $jobId]) }}" method="POST">
                    @csrf
                    ...
                    <p class="text-main-theme mt-2 fs-f20">
                        ...
                        <a href="{{ route('resume.basic-info') }}" onclick="storeJobSession('{{ $jobId }}')">登録はこちらです</a>
                    </p>
                    @endif
                    ...
                    <button type="submit" name="offer" style="all: unset; cursor: pointer;">
                        <img src="{{ asset('img/' . $offerImage) }}" class="w-100" style="max-width: 400px;" alt="オファーボタン">
                    </button>
                    @else
                    {{-- 押せない (readonly) --}}
                    <p class="text-main-theme mt-2 fs-f20">
