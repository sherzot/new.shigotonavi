    ...
    <form wire:submit.prevent="save" class="container px-3 py-4" style="max-width: 850px;">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6 offset-md-6 text-end mb-3">
                ...
                @endif
                <div wire:loading wire:target="photo" class="text-center mt-2">
                    <div class="spinner-border text-primary" role="status">
                        <span class="visually-hidden">アップロード中...</span>
                    </div>
                    <div class="text-primary small mt-2">写真アップロード中...</div>
                </div>
        ...
        {{-- 🎓 希望条件 --}}
        <div class="my-4">
            <h5 class="fw-medium text-primary">希望条件</h5>

            {{-- 希望職種（大分類） --}}
            <div class="mb-3">
                <label class="form-label">希望職種（大分類）</label>
                <select class="form-select" wire:model.lazy="hopeCondition.job_type_big_code">
                    <option value="">選択してください</option>
            ...
            {{-- 希望職種（小分類） --}}
            <div class="mb-3">
                <label class="form-label">希望職種（小分類）</label>
                <select class="form-select" wire:model.lazy="hopeCondition.job_type_small_code">
                    <option value="">選択してください</option>
                    @foreach ($this->getHopeSmallJobTypes() as $type)
            ...
            {{-- 希望勤務地（都道府県） --}}
            <div class="mb-3">
                <label class="form-label">希望勤務地（都道府県）</label>
                <select class="form-select" wire:model.lazy="hopeCondition.prefecture_code">
                    <option value="">選択してください</option>
                    @foreach ($prefectures2 as $pref)
                        <option value="{{ $pref->prefecture_code }}">{{ $pref->prefecture_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            {{-- 希望給料（時給 or 年収） --}}
            <div class="mb-3">
                <label class="form-label">希望給与 <small class="text-primary">(最低額を入力)</small></label>
                    ...
                    <div class="col-md-6">
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio"
                                wire:model="hopeCondition.salary_type"
                                value="yearly"
                                id="salaryYearlyRadio"
                                name="salary_type">
                            <label class="form-check-label text-danger" for="salaryYearlyRadio">年収：例: 400 (万円〜)</label>
                        </div>
                        <div class="input-group">
                            <input type="number" class="form-control"
                                wire:model.lazy="hopeCondition.yearly_income_min"
                                id="yearlyInput"
                            />
                    ...
                    <div class="col-md-6">
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio"
                                wire:model="hopeCondition.salary_type"
                                value="hourly"
                                id="salaryHourlyRadio"
                                name="salary_type">
                            <label class="form-check-label text-danger" for="salaryHourlyRadio">時給：例: 1200 (円〜)</label>
                        </div>
                        <div class="input-group">
                            <input type="number" class="form-control"
                                wire:model.lazy="hopeCondition.hourly_income_min"
                                id="hourlyInput"
                            />
        ...
        <div class="mb-4">
            <h5 class="text-primary mb-4">スキル</h5>

            <div class="row g-3">
                @foreach ($skillCategories as $categoryCode => $categoryLabel)
                <div class="col-md-6 col-lg-3">
                    ...
                    @error("skills.$categoryCode") <div class="text-danger small scroll-target">{{ $message }}</div> @enderror
                </div>
                @endforeach
            </div>
        </div>
        <div class="mb-4">
            ...
            <div class="card-body row g-4">
                ...
                <div class="col-md-6 d-flex align-items-center">
                    <label class="form-label mb-0 w-100">志望動機を選ぶための識別名</label>
                </div>
                <div class="col-md-6">
                    <input type="text" class="form-control border-dark" wire:model.defer="resumePreference.subject">
                    @error('resumePreference.subject') <div class="text-danger small scroll-target">{{ $message }}</div> @enderror
                </div>
        
                {{-- 希望通勤時間 --}}
                <div class="col-md-6 d-flex align-items-center">
                    <label class="form-label mb-0 w-100">希望通勤時間</label>
                </div>
                <div class="col-md-6">
                    <div class="input-group">
                ...
                {{-- 結婚状況 --}}
                <div class="col-md-6 d-flex align-items-center">
                    <label class="form-label mb-0 w-100">結婚状況</label>
                </div>
                <div class="col-md-6">
                    <select class="form-control border-dark" wire:model.defer="marriage_flag">
                        <option value="">選択してください</option>
                ...
                {{-- 扶養家族 --}}
                <div class="col-md-6 d-flex align-items-center">
                    <label class="form-label mb-0 w-100">扶養家族（配偶者を除く）</label>
                </div>
                <div class="col-md-6">
                    <input type="number" class="form-control border-dark" wire:model.defer="dependent_number">
                    @error('dependent_number') <div class="text-danger small scroll-target">{{ $message }}</div> @enderror
                </div>
        
                {{-- 配偶者の扶養義務 --}}
                <div class="col-md-6 d-flex align-items-center">
                    <label class="form-label mb-0 w-100">配偶者の扶養義務</label>
                </div>
                <div class="col-md-6">
                    <select class="form-control border-dark" wire:model.defer="dependent_flag">
                        <option value="">選択してください</option>
                ...
                <div class="col-md-6 d-flex">
                    <label class="form-label mb-0 w-100">志望動機</label>
                </div>
                <div class="col-md-6">
                    <textarea class="form-control border-dark" rows="2" wire:model.defer="resumePreference.wish_motive"></textarea>
                    <small class="text-muted">※履歴書へ表示されるのは全角200文字までです。(出力形式により多少前後します。)</small>
                ...
                {{-- 本人希望欄 --}}
                <div class="col-md-6 d-flex">
                    <label class="form-label mb-0 w-100">本人希望欄</label>
                </div>
                <div class="col-md-6">
                    <textarea class="form-control border-dark" rows="2" wire:model.defer="resumePreference.hope_column"></textarea>
                    <small class="text-muted">※履歴書へ表示されるのは全角200文字までです。(出力形式により多少前後します。)</small>
...
<script>
    ...
    document.addEventListener('livewire:load', function () {
        const yearlyRadio = document.getElementById('salaryYearlyRadio');
        const hourlyRadio = document.getElementById('salaryHourlyRadio');
        ...
        // Livewire DOM update'dan keyin qayta tekshirish
        Livewire.hook('message.processed', (message, component) => {
            toggleSalaryInputs();
        });

        // Radio buttonlar o‘zgarganda ishlatish
        yearlyRadio.addEventListener('change', toggleSalaryInputs);
        hourlyRadio.addEventListener('change', toggleSalaryInputs);

        toggleSalaryInputs(); // birinchi yuklanganda
    });
</script>
@endpush
