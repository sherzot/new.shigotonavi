...
<div class="container-fluid p-3 bg-white">
    ...
    <form action="{{ route('agent.store_job', ['companyCode' => $companyCode]) }}" method="POST">
        @csrf
        <div class="row">
            <!-- 勤務形態 -->
...
<div class="row g-3 my-3">
    <div class="col-12">
        <input type="text" class="form-control" id="public_limit_day" name="public_limit_day" value="{{ old('public_limit_day') }}" required>
    </div>
...
@for ($i = 1; $i <= 4; $i++) @php $oldQualifications=old('qualifications') ?? []; $groupOld=$oldQualifications[$i]['group_code'] ?? '' ; $categoryOld=$oldQualifications[$i]['category_code'] ?? '' ; $codeOld=$oldQualifications[$i]['code'] ?? '' ; @endphp <div class="row g-4 align-items-center mb-3">
    ...
    <div class="mb-4">
        <label for="employee_code" class="form-label fw-bold">関連担当者コード <span style="color: rgba(255, 0, 0, 0.674);">必須 例；L0000000</span></label>
