...
class AgentExport implements WithEvents
{
        ...
        public function makeDailySheet($selectDate, $event)
        {
                ...
                if ($users) {
                        ...
                        $sheet->setCellValue("M3", "登録アクション"); // 🆕 登録アクション
                        $sheet->setCellValue("N3", "経験職種");
                        $sheet->setCellValue("O3", "希望職種");
                        $sheet->setCellValue("P3", "希望勤務形態");
                        $sheet->setCellValue("Q3", "Mypage_access");
                        $sheet->getStyle("Q3")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("R3", "求人一覧表示数");
                        $sheet->getStyle("R3")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("S3", "絞込数");
                        $sheet->setCellValue("T3", "詳細閲覧数");
                        $sheet->getStyle("T3")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("U3", "オファー"); // 🆕 moved up!
                        ...
                        $sheet->getColumnDimension('M')->setWidth(16);  // 登録アクション
                        ...
                        $sheet->getColumnDimension('U')->setWidth(10);  // オファー
                } //if ($users)
                $row = 4;
                $cnt = 1;
                $tempCode = "";
                ...
                foreach ($users as $user) {
                        ...
                        $sheet->setCellValue("M{$row}", $user->register_action ?? ''); // 🆕 登録アクション
                        $sheet->setCellValue("N{$row}", $user->career_job ?? '');
                        $sheet->getStyle("N{$row}")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("O{$row}", $user->hope_job ?? '');
                        $sheet->getStyle("O{$row}")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("P{$row}", $workingStyle ?? '');
                        $sheet->setCellValue("Q{$row}", $user->mypage_at ?? '');
                        $sheet->setCellValue("R{$row}", $user->match_count ?? '');
                        $sheet->setCellValue("S{$row}", $user->update_count ?? '');
                        $sheet->setCellValue("T{$row}", $user->detail_count ?? '');
                        $sheet->setCellValue("U{$row}", $user->offer ?? ''); // オファー
                ...
                foreach ($oldUsers as $old) {
                        ...
                        $sheet->setCellValue("M{$row}", $old->register_action ?? ''); // 🆕 登録アクション
                        $sheet->setCellValue("N{$row}", $old->career_job ?? '');
                        $sheet->getStyle("N{$row}")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("O{$row}", $old->hope_job ?? '');
                        $sheet->getStyle("O{$row}")->getAlignment()->setWrapText(true);
                        $sheet->setCellValue("P{$row}", $workingStyle);
                        $sheet->setCellValue("Q{$row}", $old->mypage_at ?? '');
                        $sheet->setCellValue("R{$row}", $old->match_count ?? '');
                        $sheet->setCellValue("S{$row}", $old->update_count ?? '');
                        $sheet->setCellValue("T{$row}", $old->detail_count ?? '');
                ...
                $sheet->getStyle("A3:U{$lastRow}")->applyFromArray($styleArray);
                $firstRow = $lastRow + 2;
                $sheet->getStyle("A{$firstRow}:U{$last}")->applyFromArray($styleArray);
