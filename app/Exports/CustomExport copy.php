...
class CustomExport implements WithEvents
{
		...
		return [
			BeforeExport::class => function (BeforeExport $event) {
				...
				if ($picture) {
					...
					// 画像データをバイナリからGDリソースに変換
					//$image = imagecreatefromstring($picture->picture);
					$imageData = base64_encode($picture->picture);
					$image = base64_decode($imageData);
					$imageResource = @imagecreatefromstring($image); //20250221	
	...
	public function makePdf($file_name)
	{
		...
		// もとになるExcelを読み込み
		//$excel_file = storage_path('app/template/resume_general.xlsx');G
		...
		if (file_exists($exportPath)) {
			...
			if ($returnVar != 0) {
				...
				Log::info('PDF 変換成功: ' . implode("\n", $output));
			}
			// dd($output);
			// PDFをA4縦で全列1ページに縮小
			$resizeCommand =  "/usr/local/texlive/2024/bin/x86_64-linux/pdfjam  --trim '10mm 10mm 10mm 10mm' --clip true  --paper a4paper --scale 0.95 --outfile $outputPdf  --no-tidy --builddir /tmp   $export_pdf_path. '/resume-' . $staffCode . '.pdf' //custom_export.pdf"; //Processのときは[]
			//dd($resizeCommand);
