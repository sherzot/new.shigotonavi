...
class RegisterController extends Controller
{
    ...
    public function registration(Request $request)
    {
        ...
        try {
            ...
            // 🔹 データベースに挿入
            // DB::table('master_person')->insert($person);
            ...
            // 🔸 データベースに保存する
            // DB::table('person_userinfo')->updateOrInsert(
            //     ['staff_code' => $staffCode],
            //     $insertUserInfo
            // );
            PersonUserInfo::updateOrCreate(
                ...
                [
                    ...
                    'regist_commit' => 1,
                    'created_at' => now(),
                    // update_at ni avtomatik boot() orqali qo‘shiladi
                ]
            );
            ...
            //     return $request->job_search === 'yes'
            //         /? redirect()->route('matchings.match')->with('success', '登録成功！仕事検索ページへ移動しました。')
            //         : redirect()->route('mypage')->with('success', '登録完了！マイページへ移動しました。');
            // } catch (\Exception $e) {
            //     DB::rollBack(); // ❌ エラーが発生した場合、トランザクションはロールバックされます。
            //     Log::error("登録エラー: " . $e->getMessage());
            //     return redirect()->back()->withErrors(['error' => '登録に失敗しました。もう一度試してください。'])->withInput();
            // }
