...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // staff_code または mail_address で MasterPerson からユーザーを取得します。
        $person = MasterPerson::select('staff_code', 'regist_commit')
            ->where(function ($query) use ($request) {
                $query->where('staff_code', $request->input('staff_code'))
                    ->orWhere('mail_address', $request->input('staff_code'));
            })
            ->where('regist_commit', 1) // regist_commit = 1 の場合のみ
            ->first();

        // 人物が存在しないか、regist_commit = 1の場合はエラーを返します。
        if (!$person) {
        ...
        // personUserInfoからユーザー情報を取得する
        $personInfo = PersonUserInfo::select('staff_code', 'password', 'access_count', 'last_access_day')
            ->where('staff_code', $person->staff_code)
            ->first();

        // パスワードが一致しない、または personInfo が見つからない場合は、エラーを返します。
        if (!$personInfo || strtoupper(md5($request->input('password'))) !== $personInfo->password) {
            return back()->withErrors([
                'staff_code' => 'IDまたはパスワードが間違っています。',
            ]);
        }
    ...
    public function resetPasswordRequest(Request $request)
    {
        ...
        return redirect()->route('login')->with('status', 'パスワードを変更手順のメールが送信されました。30分以内にメールをご確認いただき、手続きを完了してください。');
    }
    /**
     * パスワードリセットフォームを表示。
     */
    ...
    public function resetPassword(Request $request)
    {
        ...
        return redirect()->route('login')->with('status', 'パスワードは正常に変更されました。');
    }
    public function deleteAccount(Request $request)
    {
        ...
        if ($user) {
            // MasterPersonテーブルのregist_commitを9に更新する
            MasterPerson::where('staff_code', $user->staff_code)->update(['regist_commit' => 9]);
            ...
            // ログインページに戻りアラートを送信する
            return redirect()->route('login')->with('message', 'アカウントが削除されました。');
        }

        return redirect()->route('mypage')->withErrors(['error' => 'アカウント削除に失敗しました。']);
    }
    /**
     * .ユーザーをログアウトします
     */
