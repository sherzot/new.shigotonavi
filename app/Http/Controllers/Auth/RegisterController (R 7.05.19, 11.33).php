...
class RegisterController extends Controller
{
    ...
    public function registration(Request $request)
    {
        ...
        try {
            ...
            // ✅ ログインログ（どの登録フローで登録したか）
            $logAction = match ($request->input('action')) {
                'view_jobs' => '求人検索',
                'create_resume' => '履歴書作成',
                default => '不明'
            };
            ...
            DB::table('log_person_signin')->insert([
             ...
             // ✅ リダイレクトするためにどのボタンが押されたかを確認します
            if ($request->action === 'create_resume') {
                return redirect()->route('resume.basic-info')->with('success', '登録完了！履歴書作成へ移動しました。');
            } else {
                return redirect()->route('matchings.match')->with('success', '登録完了！求人を表示します。');
            }
        } catch (\Exception $e) {
            DB::rollBack();
